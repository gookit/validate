# RFC：validator 入参 RV 化（`internal/validators` 核心 + FieldLevel 自定义校验器）
...
2. **扩展性**：自定义校验器**新增**一种参数形态（FieldLevel：直接拿 `reflect.Value`/字段名/args），与现有 `func(val any,...)` **共存**，为后续能力（自定义类型、跨字段、更复杂规则）留口。
...
4. **自定义校验器新增 FieldLevel 形态**：`AddValidator` 额外接受 `func(fl FieldLevel) bool`（typed，直调免 `reflect.Call`），与 `func(val any,...)` 并存（用户明确要求）。

外加（性能可选、可后置）：**端到端 RV 串联**（`tryGetRV` + carrier 持 RV），让 `CheckErr` 真正不装箱。

---

## 3. 载体（carrier）：扩展 `fieldval.FieldValue` + 公开 `FieldLevel`

- 内核继续用 `internal/fieldval.FieldValue`（已 lazy 完备）。把 `NewRV` 正式启用。
- 自定义校验器需要一个**公开**类型 → 在 root 包定义 `FieldLevel` 接口，内部由 `*fieldval.FieldValue` 实现（薄包装，或直接让 root 包一个公开 struct 持有它）。建议**最小接口**：

```go
// FieldLevel 暴露给自定义校验器的字段级上下文(不装箱拿 reflect.Value)。
type FieldLevel interface {
    Field() reflect.Value   // 去指针后的字段值(= RealV)
    Raw() reflect.Value     // 原始 reflect.Value(= RV)
    FieldName() string      // 字段路径,如 "addr.city"
    Arg(i int) (any, bool)  // 规则第 i 个参数
    Args() []any
    // 可选(跨字段/上下文): Validation() *Validation
}
```

> 决策点 D1：`FieldLevel` 是否暴露 `Validation()`（跨字段读其它字段）？go-playground 用 `Top()/Parent()`。建议 v1 先**不**暴露 `*Validation`（避免把内部全摊开），跨字段需求仍走 context validator（§4）；后续按需加。
...
## 7. 自定义校验器：新增 FieldLevel 形态（additive）

保留旧：`AddValidator("n", func(val any, a1 T1, ...) bool)`（reflect.Call，原样）。
新增：`AddValidator("n", func(fl FieldLevel) bool)`（**typed、直调、免 reflect.Call**）。

实现要点：
1. `checkValidatorFunc`（util.go）识别 func 形态：若入参恰为 `FieldLevel` 接口类型 → 记为 **fieldlevel 风格**；否则走旧 `any`/typed 反射风格。
2. `funcMeta` 加判别字段（如 `style uint8`：legacy-reflect / fieldlevel）；fieldlevel 风格**额外存一个 typed 字段** `flFunc func(FieldLevel) bool`（避免 reflect.Call 的 argIn 装箱）。
3. `callValidatorValue`（default 分支）按 style 分派：
   - fieldlevel：`ok = fm.flFunc(flWrapper)`（直调；args 经 `fl.Arg(i)` 取）。
   - legacy：现有 `reflect.Call` 路径不变。

> 决策点 D2：fieldlevel 风格的 args 经 `fl.Arg(i)` 取（运行期 `any`），还是也支持 typed 形参？建议 v1 只支持 `func(FieldLevel) bool`（args 走 `fl.Arg/Args`），最简、免反射；typed-args 留给旧风格。
> 决策点 D3：是否同时给 `AddValidators`/struct 内方法形自定义校验器支持 FieldLevel？建议先只在 `AddValidator`/`v.AddValidator` 支持，struct 方法形后续再说。
...
| 扩展性 / 自定义能力 | any | — | **FieldLevel 自定义校验器可用** |
...
| **R3** | 公开 `FieldLevel` + `checkValidatorFunc` 识别新签名 + `funcMeta.style` + `callValidatorValue` 分派；新增 `func(FieldLevel) bool` 自定义校验器支持 | 中 | **新增 API（附加）** | 不变 |
...
- **D1** `FieldLevel` 是否暴露 `Validation()`/跨字段访问？（建议 v1 不暴露）
- **D2** 新自定义签名是否只 `func(FieldLevel) bool`（args 走 `fl.Arg`）？（建议是）
...
- **D6** 命名：`FieldLevel`（对齐 go-playground）还是 `FieldValue`(公开) / `Field`？
