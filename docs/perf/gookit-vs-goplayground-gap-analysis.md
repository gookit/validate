...
> **更新（2026-06）**：RV-native 重构已落地——~60+ 无状态内建校验器搬入 `internal/validators`（入参封装的 `reflect.Value`）、热路径直调去二次反射、新增 `func(FieldCtx) bool` 自定义校验器，并端到端去装箱。本文「CheckErr ~3 alloc 下限 / T3 去装箱未做」等结论已被超越：**`CheckErr` 结构体通过路径已达 0 alloc**（原 3）；`Check`/`Struct().Validate()` 因 safeData 收集仍 6/12（功能取舍，不变）。详见 [`rv-native-validators-rfc.md`](rv-native-validators-rfc.md)。
