...
### T2. 把常用字符串校验器搬进 `callValidator` 的 switch（免反射 Call）
...
### T5. `safeData` / `Errors` 结果 map 懒分配 + opt-in 关收集
...
### T6. `Translator` 两个 map（labelMap/fieldMap）懒分配
...
3. **T4 + T6 + T5(懒分配部分)** 合并为"**`newEmpty`/`Translator` 全面懒分配**"专项 —
   默认可改、风险可控，统一把 Errors/safeData/filteredData/labelMap/fieldMap 改懒分配，
   合计可省约 5–6 alloc（成功路径）。
